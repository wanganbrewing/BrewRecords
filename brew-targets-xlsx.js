(function(root,factory){
  ...
  const FIELD_DEFS=[
    ...
    ['仕上がり','targetFG','目標FG','number','SG'],['仕上がり','targetABV','目標ABV（自動計算）','number','%'],['仕上がり','targetIBU','目標合計IBU（自動計算）','number','IBU'],['仕上がり','targetSRM','目標SRM','number','SRM'],['仕上がり','planNotes','仕込み計画メモ','text',''],
    ['酵母','yeastSource','酵母の由来','text',''],['酵母','yeastHarvestDate','酵母回収予定日','date',''],['酵母','cellDensity','細胞密度 目標','number','×10⁶ cells/mL']
  ];
  const WATER_DEFS=[
    ['水量','mashWater1','糖化用水 1回目','number','L','extra'],['水量','mashWater2','糖化用水 2回目','number','L','extra'],['水量','spargeWater1','スパージ水 1回目','number','L','extra'],['水量','spargeWater2','スパージ水 2回目','number','L','extra'],['水量','waterVolume','糖化用水 合計','number','L','bound']
  ];
  const MATERIAL_HEADERS=['行種別','分類・区分','名称・項目','重さ・値','2回目の重さ','単位','在庫品目','α酸（%）','投入方法・先','投入時期','IBU（自動計算）','濃度（%）','管理キー','在庫ID（参考）'];
  ...
  function planFrom(batch){assertReady();const plan=BrewTargets.normalize(batch?.brewTargets),ibu=BrewTargets.autoIbuRows(batch?.hops,batch?.batchSize,batch?.targetOG);plan.fields.targetABV=calculatedTargetAbv(batch?.targetOG,plan.fields.targetFG);plan.fields.targetIBU=ibu.total;return plan;}
  ...
  function materialRows(batch,plan,inventory){
    ...
    for(const type of ['fermentable','hop','adjunct']){const [arrayKey,defaultUnit]=BrewTargets.rowTypes[type];for(const row of batch?.[arrayKey]||[]){
      const auto=type==='hop'?BrewTargets.hopIbu(row,batch?.batchSize,batch?.targetOG):'',meta=BrewTargets.rowMeta(row),inv=inventoryMeta(inventory,row),unit=defaultUnit;
      rows.push(['原材料',CATEGORY_LABELS[type],row.name||'',asExcelValue(meta.batch1,'number'),asExcelValue(meta.batch2,'number'),unit,inv.name,asExcelValue(meta.alpha,'number'),row.timingType||row.timing||'',asExcelValue(row.timingValue,'number'),asExcelValue(auto,'number'),asExcelValue(meta.concentration,'number'),type,row.invId||'']);
    }}
    if(trim(batch?.yeast)||trim(batch?.yeastAmount)){
      ...
      rows.push(['原材料',CATEGORY_LABELS.yeast,batch.yeast||'',asExcelValue(batch.yeastAmount,'number'),'','g',inv.name,'','','','','','yeast',batch.yeastInvId||'']);
    }
    return rows;
  }
  ...
  function buildWorkbook(batch,inventory=[],options={}){
    ...
    const materials=makeSheet(xlsx,[MATERIAL_HEADERS,...materialRows(batch,plan,inventory)],[13,15,28,18,18,13,28,13,18,15,18,13,18,22]);
  ...
  function importFieldRows(rows,batch,plan,xlsx){
    ...
    const seen=new Set();
    for(const row of rows){const key=trim(row['管理キー']);if(!key)continue;if(!defs.has(key))continue;if(seen.has(key))throw Error(`管理キー「${key}」が重複しています。`);seen.add(key);const def=defs.get(key),value=valueFor(row['値']??row['重さ・値'],def[3],xlsx,def[2]),scope=def[5]||(!FIELD_DEFS.some(d=>d[1]===key)?'bound':'auto');
  ...
  function importMaterials(rows,batch,plan,inventory,xlsx,warnings){
    ...
    for(const row of rows.filter(item=>trim(item['行種別'])==='原材料')){
      ...
      const name=trim(row['名称・項目']),unit=trim(row['単位']);
      const batch1=valueFor(row['重さ・値'],'number',xlsx,`${name||label}の量`),batch2=valueFor(row['2回目の重さ'],'number',xlsx,`${name||label}の量`);
      if(type==='yeast'){
        ...
        const inventoryName=trim(row['在庫品目']);if(inventoryName){const matches=(inventory||[]).filter(item=>item.category==='yeast'&&trim(item.name).toLocaleLowerCase()===inventoryName.toLocaleLowerCase());if(matches.length===1&&matches[0].unit===batch.yeastUnit)batch.yeastInvId=matches[0].id;else warnings.push(`酵母「${name}」は在庫と未連携で読み込みます。`);}
        continue;
      }
      const meta={batch1,batch2,alpha:valueFor(row['α酸（%）'],'number',xlsx,'α酸'),ibu:'',concentration:valueFor(row['濃度（%）'],'number',xlsx,'濃度')};
      const material={name,unit:BrewTargets.rowTypes[type][1],timingType:trim(row['投入方法・先'])||'boil',timing:trim(row['投入方法・先']),timingValue:valueFor(row['投入時期'],'number',xlsx,'投入時期'),targetMeta:meta};
      if(type!=='mineral'&&name&&trim(row['在庫品目'])){const inventoryName=trim(row['在庫品目']),matches=(inventory||[]).filter(item=>item.category===type&&trim(item.name).toLocaleLowerCase()===inventoryName.toLocaleLowerCase());if(matches.length===1&&(type!=='adjunct'||matches[0].unit===material.unit))material.invId=matches[0].id;else warnings.push(`${label}「${name}」は在庫と未連携で読み込みます。`);}
  ...
  function parseWorkbook(data,inventory=[],options={}){
    ...
    const plan=BrewTargets.empty(),warnings=[];importFieldRows(basic,batch,plan,xlsx);importMaterials(materials,batch,plan,inventory,xlsx,warnings);importProcess(process,batch,plan);plan.fields.doubleBrew=materials.some(row=>trim(row['2回目の重さ'])!=='');plan.fields.targetABV=calculatedTargetAbv(batch.targetOG,plan.fields.targetFG);const ibu=BrewTargets.autoIbuRows(batch.hops,batch.batchSize,batch.targetOG);batch.hops=ibu.rows;plan.fields.targetIBU=ibu.total;batch.brewTargets=BrewTargets.waterPlan(BrewTargets.normalize(plan),batch.waterVolume);
