...
function openExpenseEditor(batchId){
  ...
  expenseEditorState={batchId,before:JSON.stringify(window.fermentCloudData.getSnapshot()),removed:[]};
...
function costHistoryHtml(batch){
  ...
  return `<details><summary>その他の原価の変更履歴（${history.length}件）</summary>${[...history].reverse().map(h=>`<div class="expense-row"><p>${escapeHtml(h.recordedAt)} ／ ${escapeHtml(h.reason)}</p><p>入力確認：${h.before?.reviewed?'済み':'未確認'} → ${h.after?.reviewed?'済み':'未確認'}</p>${[['変更前',h.before],['変更後',h.after]].map(([label,state])=>`<strong>${label}</strong><ul>${(state?.rows||[]).map(r=>`<li>${escapeHtml(BatchExpenses.categories[r.category]||r.category)}：${escapeHtml(r.name)} ／ 元の金額 ${yenReference(r.amount===''?null:r.amount)} × ${escapeHtml(r.percent)}% ／ ${escapeHtml(r.date||'日付なし')} ／ ${escapeHtml(r.reference||'')} ／ ${escapeHtml(r.note||'')}</li>`).join('')||'<li>追加費用なし</li>'}</ul>`).join('')}</div>`).join('')}</details>`;
}
function renderExpenseSummary(batch,material){
  ...
  return `<div class="cost-summary"><div>原材料費${material.complete?'':'（計算できた分）'}<strong>${yenReference(material.subtotal)}</strong></div><div>その他の原価${sum.extra.unknown?'（計算できた分）':''}<strong>${yenReference(sum.extra.subtotal)}</strong></div><div>${sum.complete?'登録範囲の原価合計':'計算できた分の小計'}<strong>${yenReference(sum.total)}</strong>${sum.complete?'':'未確定'}</div></div>
    ...
    <p class="inventory-table-note">原材料費は消費記録から計算します。その他は下の入力で割り当てた費用のみです。未登録の費用は合計に含まれません。会計・申告用の確定原価ではありません。</p>
