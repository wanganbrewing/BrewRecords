...
function targetStepActual(step,b,batchId=targetSheetBatchId){
  ...
  return `<td class="target-mobile-actual" data-label="実績"><span>${targetEsc(saved?targetActualSummary(step,b):'仕込み計画を保存すると入力できます。')}</span>${saved?`<button type="button" class="btn btn-primary" data-target-process-actual="${targetEsc(batchId)}" data-target-process-stage="${targetEsc(step.name)}">実績を入力</button>`:''}</td>`;
}
