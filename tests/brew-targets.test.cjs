...
test('unified plan exposes the requested current fields and omits retired input controls',()=>{
  ...
  for(const text of ['スタイル・目標・実績','スタイルを選ぶと参考値を表示します','酵母の使用量はgで入力します','実績を入力','酒税法上の品目区分','初期設定','目標IBU（自動計算）'])assert.ok(ui.includes(text));
  assert.ok(!ui.includes('この工程の実績を入力'));
