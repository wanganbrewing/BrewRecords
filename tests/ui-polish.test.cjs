...
function harness(){const els={editorStatus:{hidden:true,textContent:''},screenHeading:{},workflowDeviceHint:{hidden:true,dataset:{}},workflowDeviceIcon:{},workflowDeviceText:{}},buttons=[{}];const c=vm.createContext({document:{addEventListener(){},getElementById:id=>els[id],querySelectorAll:()=>buttons},formIsOpen:true,saveBatch:async()=>{},cancelForm(){},confirm:()=>false});vm.runInContext(source,c);return {c,els,buttons};}
test('screen headings describe the task and show the recommended input device',()=>{const {c,els}=harness();c.updateScreenChrome('form');assert.equal(els.screenHeading.textContent,'仕込みの内容を入力する');assert.equal(els.workflowDeviceText.textContent,'PCでの仕込み計画入力に最適');assert.equal(els.workflowDeviceHint.dataset.mode,'pc');assert.equal(els.editorStatus.hidden,true);c.updateScreenChrome('schedule');assert.equal(els.workflowDeviceHint.dataset.mode,'hybrid');assert.match(els.workflowDeviceText.textContent,/工程目標はPC向き・実績入力はスマホ向き/);c.updateScreenChrome('fermentation');assert.equal(els.workflowDeviceHint.dataset.mode,'mobile');assert.match(els.workflowDeviceText.textContent,/スマホ/);c.updateScreenChrome('tanks');assert.equal(els.screenHeading.textContent,'タンクの清掃状態を管理する');assert.match(els.workflowDeviceText.textContent,/清掃・殺菌/);c.updateScreenChrome('list');assert.equal(els.workflowDeviceHint.hidden,true);c.updateScreenChrome('inventory');assert.equal(els.screenHeading.textContent,'在庫と入荷・使用を管理する');});
