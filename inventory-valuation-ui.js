...
function renderBatchMaterialCost(batch){
  ...
  return `<details class="section"><summary class="section-title">原材料費（参考）</summary><p>${cost.complete?'消費記録の原材料費':'計算できた分の小計'}：${yenReference(cost.subtotal)}${cost.complete?'':' ／ 未確定'}</p><p class="inventory-table-note">消費記録の数量と消費時点の平均単価で計算します。未連携・未消費・数量不一致・金額未入力は確定しません。洗浄剤・光熱費・人件費・包装費・水質添加剤は含みません。後の入荷訂正で再計算されます。</p>${cost.missing.length?`<p class="inv-unlinked-warning">要確認：${cost.missing.map(escapeHtml).join('、')}</p>`:''}<div class="detail-list">${cost.uses.map(r=>`<div class="row"><div>${escapeHtml(inventoryItemLabel(r))}<br>${escapeHtml(r.date)} ／ ${escapeHtml(r.amount)} ${escapeHtml(r.unit)}${r.reason?'<br>'+escapeHtml(r.reason):''}</div><span>${yenReference(r.value)}</span></div>`).join('')||'<p>消費記録がありません。仕込み側で在庫と連携し、「在庫から消費を記録」を行ってください。</p>'}</div></details>`;
}
